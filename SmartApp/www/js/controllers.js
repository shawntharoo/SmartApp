angular.module('app.controllers', []).controller('orderPizzaCtrl', function(
        $scope, $state) {
        //$tbit= false;
        //$gbit=true;
    ...
    }).controller('cartCtrl', function($scope) {}).controller('loginCtrl',
...
*/
            $scope.login = function(email, password) {
                $http.get('http://localhost/test/logcheck.php?email=' +
                    email + '&password=' + password).then(function(
                    response) {
                    ...
                    if (role == "admin") {
                        window.localStorage.setItem("username",
                            email); //set username to localstorage
                        ...
                        $state.transitionTo(
                            'AdmintabsController.mainprofile', {
                                name: role
                            });
                    } else if (role == "member") {
                        window.localStorage.setItem("username",
        ...
        }).controller('timelineCtrl', function($scope) {}).controller(
        'resetpasswordCtrl', function($scope) {}).controller('signupCtrl',
...
*/
            $scope.namecheck = function() {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    if ((name > 64 && name < 91) || (name > 96 && name <
                        123)) {
                        Check = 0;
                    } else {
                        Check = 1;
...
*/
            $scope.signup = function(name, myclass, email) {
                if (angular.isUndefined(name) || name === null || angular.isUndefined(
                    myclass) || myclass === null || email === null) {
                    var alertPopup = $ionicPopup.alert({
                        title: 'Error',
                    ...
                    $http.get('http://localhost/test/signup.php?name=' +
                            name + '&class=' + myclass + '&email=' + email)
        ...
        $http.get('http://localhost/test/getname.php?id=' + x).then(
            function(response) {
        ...
        $http.get('http://localhost/test/getspouse.php?id=' + x).then(
            function(response) {
                var records = response.data;
                for (var i = 0; i < records.length; i++) {
                    var record = records[i];
        ...
        $http.get('http://localhost/test/getchildcount.php?id=' + x).then(
            function(response) {
                var records = response.data;
                for (var i = 0; i < records.length; i++) {
                    var record = records[i];
...
*/
        ...
        $http.get('http://localhost/test/getaddress.php?user=' + z).then(
            function(response) {
...
*/
        $scope.addpersonaldetails = function(address, home, mobile) {
            ...
            if (angular.isUndefined(address) || address === null ||
                angular.isUndefined(home) || home === null || angular.isUndefined(
                    mobile) || mobile === null) {
                var alertPopup = $ionicPopup.alert({
                    title: 'Error',
                ...
                $http.get('http://localhost/test/personal.php?address=' +
                    address + '&home=' + home + '&mobile=' + mobile +
                    '&id=' + y).then(function(response) {
                    if (response.data == "true") {
                        var alertPopup = $ionicPopup.alert({
                            title: 'SmartApp',
    ...
    }).controller('requestCtrl', function($scope, $http, $ionicPopup, $state,
        $window) {
        $http.get('http://localhost/test/memberRequest.php').then(function(
            response) {
...
*/
        $scope.approve = function(id) {
            $http.get('http://localhost/test/acceptRequest.php?id=' +
...
*/
        $scope.reject = function(id) {
            $http.get('http://localhost/test/rejectRequest.php?id=' +
                id).then(function(response) {
                if (response.data == "true") {
                    var alertPopup = $ionicPopup.alert({
                        title: 'SmartApp',
    ...
    }).controller('profileEditCtrl', function($scope, $http, $ionicPopup,
        $window, $state) {
        /*
The code below will check the 
in put field for incorrect
...
*/
        ...
        $scope.profileEdit = function(name, email) {
            if (angular.isUndefined(name) || name === null || email ===
                null) {
                var alertPopup = $ionicPopup.alert({
                    title: 'Error',
                    template: 'Plese enter all the fiedls'
                ...
                var x = window.localStorage.getItem("id");
                $http.get('http://localhost/test/profileEdit.php?id=' +
                    x + '&name=' + name + '&email=' + email).then(
                    function(response) {
                        if (response.data == "true") {
                            var alertPopup = $ionicPopup.alert({
                                title: 'SmartApp',
    ...
    }).controller('request1Ctrl', function($scope) {}).controller(
        'occupationCtrl', function($scope) {}).controller('familyCtrl',
        function($scope, $http, $ionicPopup, $state) {
            /*
The code below will check the 
in put field for incorrect
...
*/
            ...
            $scope.changesname = function(sname) {
                ...
                if (angular.isUndefined(sname) || sname == null || document
                    .getElementById('name').value === null) {
                    var alertPopup = $ionicPopup.alert({
                        title: 'SmartApp',
                    ...
                    $http.get('http://localhost/test/schange.php?id=' + r +
                        '&sname=' + sname).then(function(response) {
                        if (response.data == "true") {
                            var alertPopup = $ionicPopup.alert({
                                title: 'SmartApp',
            ...
            $scope.add = function(cname, c) {
                    ...
                    var r = window.localStorage.getItem("id");
                    if ((angular.isUndefined(cname) || cname === null ||
                        angular.isUndefined(c) || c === null)) {
                        var alertPopup = $ionicPopup.alert({
                            title: 'SmartApp',
                        ...
                        $http.get('http://localhost/test/childadd.php?id=' + r +
                            '&cname=' + cname + '&dob=' + date1 + '&royal=' +
            ...
            $scope.edit = function(a) {
                select = a;
                $http.get('http://localhost/test/getchildren1.php?id=' + a)
                    .then(function(response) {
            ...
            $scope.delete = function(b) {
                $http.get('http://localhost/test/delchildren.php?id=' + b).then(
                    function(response) {
                        $http.get(
                            'http://localhost/test/getchildren.php?id=' +
                            u).then(function(response) {
                            $scope.childlist = response.data;
                        });
                    });
            }
            $http.get('http://localhost/test/getchildren.php?id=' + u).then(
                function(response) {
                    $scope.childlist = response.data;
                });
            $scope.editchild = function(a, b) {
                ...
                var year = date.getFullYear();
                var date1 = year + "-" + month + "-" + day;
                if ((angular.isUndefined(a) || b === null || angular.isUndefined(
                    b) || b === null)) {
                    var alertPopup = $ionicPopup.alert({
                        title: 'SmartApp',
                        template: 'Please enter all the fields'
                    ...
                    $http.get('http://localhost/test/childupd.php?id=' +
                        select + '&cname=' + a + '&dob=' + date1 +
                        '&royal=' + x).then(function(response) {
                        if (response.data == "true") {
                            var alertPopup = $ionicPopup.alert({
                                title: 'SmartApp',
    ...
    .controller('cricketPostCtrl', function($scope, $state, $http) {
        ...
        $scope.$on('$ionicView.enter', function() {
            ...
            $http.get('http://localhost/test/subposts.php?id=' +
                    window.localStorage.getItem("selectitemid") +
                    '&UID=' + UID).then(function(response) {
            ...
            $http.get('http://localhost/test/subgrp.php?id=' +
                window.localStorage.getItem("selectitemid") +
                '&UID=' + UID).then(function(response) {
        ...
        $scope.searchlist = function(searching) {
                if (searching == null) {
                    $http.get('http://localhost/test/getgroups5.php?UID=' +
                        UID).then(function(response) {
                        $scope.names = response.data;
                    })
                } else {
                    $http.get('http://localhost/test/getgroups3.php?name=' +
                        searching + '&UID=' + UID).then(function(
                        response) {
                        $scope.names = response.data;
                    })
                }
        ...
        $scope.searchlist = function(searching) {
                if (searching == null) {
                    $http.get('http://localhost/test/getgroups.php?UID=' +
                        UID).then(function(response) {
                        $scope.names = response.data;
                    })
                } else {
                    $http.get('http://localhost/test/serachgroup.php?name=' +
                        searching + '&UID=' + UID).then(function(
                        response) {
                        $scope.names = response.data;
                    })
    ...
    }).controller('followCricketCtrl', function($scope, $state, $http,
        $ionicPopup) {
        var UID = window.localStorage.getItem("id");
        //load group details
        $scope.$on('$ionicView.enter', function() {
            //get the details about the group members from the details
            $http.get('http://localhost/test/subgrpmember.php?id=' +
                window.localStorage.getItem("selectitemiid") +
                '&UID=' + UID).then(function(response) {
                $scope.names = response.data;
        ...
        $scope.getid = function(x) {
            window.localStorage.setItem("selectitemid", x);
            $http.get('http://localhost/test/follow.php?id=' + x +
                '&UID=' + UID).then(function(response) {
                var alertPopup = $ionicPopup.alert({
                    title: 'Follow group',
                    template: 'Now you are a member of this group'
                });
                $http.get('http://localhost/test/updatenot.php')
                    .then(function(response) {});
        ...
        $scope.addpost = function(a, b) {
            if (a != null && b != null) { // check that post details are added or not
                $http.get('http://localhost/test/addpost.php?id=' +
                    window.localStorage.getItem("selectitemid") +
                    '&description=' + a + '&image=' + b + '&UID=' +
                    UID).then(function(response) {
                    var alertPopup = $ionicPopup.alert({
                    ...
                    $http.get(
                        'http://localhost/test/updatenot.php'
                    ).then(function(response) {});
                    $state.transitionTo('cricketPost');
                });
            } else {
                ...
                $http.get('http://localhost/test/updatenot.php').then(
                    function(response) {});
            }
        };
    }).controller('adminCreateAGroupCtrl', function($scope, $http, $ionicPopup,
        $state) {
        var UID = window.localStorage.getItem("id");
        $scope.createGroup = function() {
        ...
        $scope.create = function(a, b, c, d) {
            if (a != null && b != null && c != null && d != null) { // check that post details are added or not
                ...
                $http.get(
                    'http://localhost/test/checkgroupname.php?name=' +
                    a + '&UID=' + UID).then(function(response) {
                    ...
                    if (Array.isArray($scope.id)) { //check group name is already avilable
                        ...
                        $http.get(
                            'http://localhost/test/addgroupname.php?name=' +
                            a + '&Description=' + b +
                            '&category=' + c + '&image=' +
                            d + '&UID=' + UID).then(
                            function(response) {
        ...
        $scope.$on('$ionicView.enter', function() {
            $http.get(
                'http://localhost/test/serachgrouppost.php?UID=' +
                UID).then(function(response) {
                $scope.names = response.data;
            })
        });
        ...
        $scope.addpost = function(a, b) {
            if (a == "Say something" && b == "Image") { // check that post details are added or not
                $http.get(
                    'http://localhost/test/addpost2.php?description=' +
                    a + '&image=' + b + '&UID=' + UID).then(
                    function(response) {
                        var alertPopup = $ionicPopup.alert({
                            title: 'User -add new posts',
                            template: 'New post has been added successfully'
        ...
        $scope.getid = function(x) {
            window.localStorage.setItem("selectitemiid", x);
            $http.get('http://localhost/test/unfollow.php?id=' + x +
                '&UID=' + UID).then(function(response) {
                var alertPopup = $ionicPopup.alert({
    ...
    }).controller('inviteFriendsCtrl', function($scope) {}).controller(
        'selectedGroupPostCtrl', function($scope) {}).controller(
        'followAGroupCtrl', function($scope, $state) {
            $scope.addPost = function() {
                $state.transitionTo("addAPost");
            }
        }).controller('NotificationsCtrl', function($scope) {}).controller(
        'adminCreateCommitteeCtrl', function($scope, $state) {}).controller(
        'footballCtrl', function($scope) {}).controller('groupSearchDemoCtrl',
        function($scope) {}).controller('selectedPostCtrl', function($scope) {})
    .controller('selectedPostBoxingCtrl', function($scope) {}).controller(
        'selectedPostDemoCtrl', function($scope) {})
    //sanda controllers
  ...
  $state.go('AdmintabsController.ufullEditShow',{id:IDAdd});
    ...
    .controller('pageCtrl', function($scope, $state) {
        ...
        $http.get('http://localhost/test/searchresult.php?name=' + name +
            '&class=' + class1 + '&profession=' + profession +
            '&country=' + country).then(function(response) {
        ...
        $http.get('http://localhost/test/searchgroupresult.php?name=' +
            name + '&class=' + class1 + '&group=' + group).then(
            function(response) {
                $scope.searchItems = response.data;
            });
    })
    ...
    .controller('favouriteContactsCtrl', function($scope, $http) {
        ...
        $http.get('http://localhost/test/favouriteList.php?id=' + id).then(
            function(response) {
                $scope.favourite = response.data;
            });
    }).controller('ViewFavouriteContactsCtrl', function($scope) {}).controller(
        'themeSettingsCtrl', function($scope) {
    ...
    .controller('searchResultCtrl', function($scope, $http, $ionicPopup) {
        ...
        var id = window.localStorage.getItem("id");
        $http.get("http://localhost/test/followpage.php?follower=" +
            follower).success(function(data) {
            var fav = [];
            fav = data;
        ...
        $http.get('http://localhost/test/checkFollow.php?follower=' +
            follower + '&id=' + id).then(function(response) {
            if (response.data == "true") {
                alert("ufollow");
        ...
        $scope.follow = function() {
            $http.post(
                "http://localhost/test/followdatabase.php?follower=" +
                follower + "&id=" + id).success(function(data) {
                var alertPopup = $ionicPopup.alert({
    ...
    .controller('eventsCtrl', function($scope, $state, $http) {
        ...
        $scope.addE = function() {
            $http.get('http://localhost/test/addEvent.php').then(
                function(response) {
        ...
        $scope.eventNames = [{
            ...
            date: "6th June 2016",
            image: "img/img3.jpg",
            assoc: "1994 Old Boys Association"
        }];
    }).controller('eventsubgroup', function($scope) {
        $scope.eventNames = [{
            name: "Save Wilpattu",
            date: "19th May 2016",
            image: "img/savewil.jpg",
            assoc: "Doctors Association"
        }, {
            name: "'Memories' 13C Batch Party",
            date: "30th May 2016",
            image: "img/img2.jpg",
            ...
            date: "27th May 2016",
            image: "img/img1.jpg",
            assoc: "1994 Charity Organization"
        }];
    ...
    }).controller('bdaymsg', function($scope) {}).controller('calendar',
        function($scope) {
            $scope.calendarVals = [{
                d1: "01",
                d2: "02",
            ...
            $scope.eventCal = {
                ...
                stime: "12:00pm",
                etime: "05:00pm",
                location: "Kingsbury, Colombo",
                description: "get the gifts ready"
            };
 ...
 */
        $scope.Birthdays = [{
            name: "Kasun Senevirathne",
            day: "Today",
            image: "img/img4.jpg"
        }, {
            name: "Malinda Samarathunga",
            day: "Today",
            image: "img/img5.jpg"
        }];
        $scope.neweventName = [{
